...
export default function App() {
  ...
  const evaluateAndGetFeedback = useCallback(async (userLandmarks, referenceLandmarks) => {
    ...
    // YORUM: Arka uç API'sini çağırma kısmı. Bu API'nin çalışması için bir sunucunun çalışıyor olması gerekir.
    // Backend'i başlatmadıysanız bu kısmı yorumdan çıkarabilirsiniz.
    /*
    try {
      const response = await fetch(`${API_URL}/evaluate_pose`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          user_pose: userLandmarks,
          reference_pose: referenceLandmarks,
        }),
      });
      const data = await response.json();
      if (data.feedback) {
