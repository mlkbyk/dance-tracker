...
if not api_key:
    # Hata: API anahtarı yoksa bir hata fırlat
...
def evaluate_pose():
    ...
    if not user_pose or not reference_pose:
        print("Hata: Gelen poz verisi eksik.")
        return jsonify({'error': 'Kullanıcı veya referans poz verisi eksik'}), 400

    print("\n----------------------------------")
    print("Poz Değerlendirme İsteği Alındı.")

    # YORUM: Anahtar açıları hesapla
    # Python tarafında koordinatlara doğrudan erişim için sözlük yapısını kullanıyoruz
    user_angles = {
    ...
    # YORUM: Yeni, daha kısa ve rol tabanlı bir prompt oluştur
    prompt = f"""
    ...
    Sağ Dirsek: {user_angles['right_elbow']:.2f} derece
    Sol Dirsek: {user_angles['left_elbow']:.2f} derece
    Sağ Omuz: {user_angles['right_shoulder']:.2f} derece
    Sol Omuz: {user_angles['left_shoulder']:.2f} derece
    Sağ Diz: {user_angles['right_knee']:.2f} derece
    Sol Diz: {user_angles['left_knee']:.2f} derece

    Referans Pozu Açıları:
    Sağ Dirsek: {reference_angles['right_elbow']:.2f} derece
    Sol Dirsek: {reference_angles['left_elbow']:.2f} derece
    Sağ Omuz: {reference_angles['right_shoulder']:.2f} derece
    Sol Omuz: {reference_angles['left_shoulder']:.2f} derece
    Sağ Diz: {reference_angles['right_knee']:.2f} derece
    Sol Diz: {reference_angles['left_knee']:.2f} derece
    """
